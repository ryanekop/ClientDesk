-- ========================================================
-- Migration: Package default and fixed operational costs
-- ========================================================

BEGIN;

ALTER TABLE public.services
ADD COLUMN IF NOT EXISTS default_operational_costs JSONB NOT NULL DEFAULT '[]'::jsonb;

UPDATE public.services
SET default_operational_costs = '[]'::jsonb
WHERE default_operational_costs IS NULL
   OR jsonb_typeof(default_operational_costs) <> 'array';

ALTER TABLE public.profiles
ADD COLUMN IF NOT EXISTS finance_fixed_operational_costs JSONB NOT NULL DEFAULT '[]'::jsonb;

UPDATE public.profiles
SET finance_fixed_operational_costs = '[]'::jsonb
WHERE finance_fixed_operational_costs IS NULL
   OR jsonb_typeof(finance_fixed_operational_costs) <> 'array';

CREATE OR REPLACE FUNCTION public.cd_guard_service_default_operational_costs_admin()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF COALESCE(NEW.default_operational_costs, '[]'::jsonb) <> '[]'::jsonb
       AND NOT public.cd_current_user_is_admin() THEN
      RAISE EXCEPTION 'Only admins can set package default operational costs.';
    END IF;
    RETURN NEW;
  END IF;

  IF COALESCE(NEW.default_operational_costs, '[]'::jsonb)
     IS DISTINCT FROM COALESCE(OLD.default_operational_costs, '[]'::jsonb)
     AND NOT public.cd_current_user_is_admin() THEN
    RAISE EXCEPTION 'Only admins can update package default operational costs.';
  END IF;
...
DROP TRIGGER IF EXISTS trg_guard_service_default_operational_costs_admin ON public.services;
CREATE TRIGGER trg_guard_service_default_operational_costs_admin
BEFORE INSERT OR UPDATE OF default_operational_costs ON public.services
FOR EACH ROW
EXECUTE FUNCTION public.cd_guard_service_default_operational_costs_admin();

CREATE OR REPLACE FUNCTION public.cd_guard_profile_fixed_operational_costs_admin()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF COALESCE(NEW.finance_fixed_operational_costs, '[]'::jsonb) <> '[]'::jsonb
       AND NOT public.cd_current_user_is_admin() THEN
      RAISE EXCEPTION 'Only admins can set fixed operational costs.';
    END IF;
    RETURN NEW;
  END IF;

  IF COALESCE(NEW.finance_fixed_operational_costs, '[]'::jsonb)
     IS DISTINCT FROM COALESCE(OLD.finance_fixed_operational_costs, '[]'::jsonb)
     AND NOT public.cd_current_user_is_admin() THEN
    RAISE EXCEPTION 'Only admins can update fixed operational costs.';
  END IF;
...
DROP TRIGGER IF EXISTS trg_guard_profile_fixed_operational_costs_admin ON public.profiles;
CREATE TRIGGER trg_guard_profile_fixed_operational_costs_admin
BEFORE INSERT OR UPDATE OF finance_fixed_operational_costs ON public.profiles
FOR EACH ROW
EXECUTE FUNCTION public.cd_guard_profile_fixed_operational_costs_admin();

CREATE OR REPLACE FUNCTION public.cd_normalize_fixed_operational_costs(
  p_fixed_costs JSONB
) RETURNS JSONB
LANGUAGE sql
STABLE
...
AS $$
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'id', COALESCE(NULLIF(item.value ->> 'id', ''), md5(item.value::text)),
      'label', btrim(COALESCE(item.value ->> 'label', item.value ->> 'name', '')),
      'amount', GREATEST(COALESCE((item.value ->> 'amount')::numeric, 0), 0),
      'frequency', CASE WHEN item.value ->> 'frequency' = 'yearly' THEN 'yearly' ELSE 'monthly' END,
      'starts_on', COALESCE(NULLIF(item.value ->> 'starts_on', '')::date, CURRENT_DATE),
      'ends_on', NULLIF(item.value ->> 'ends_on', '')::date,
      'is_active', COALESCE((item.value ->> 'is_active')::boolean, TRUE)
    )
  ), '[]'::jsonb)
  FROM jsonb_array_elements(
    CASE
      WHEN jsonb_typeof(p_fixed_costs) = 'array' THEN p_fixed_costs
      ELSE '[]'::jsonb
    END
  ) AS item(value)
  WHERE btrim(COALESCE(item.value ->> 'label', item.value ->> 'name', '')) <> ''
    AND GREATEST(COALESCE((item.value ->> 'amount')::numeric, 0), 0) > 0;
$$;
...
AS $$
  ...
  fixed_cost_events AS (
    SELECT
      fixed_cost.value ->> 'id' AS fixed_cost_id,
      CASE
        WHEN fixed_cost.value ->> 'frequency' = 'yearly'
          THEN COALESCE((fixed_cost.value ->> 'amount')::numeric, 0) / 12
        ELSE COALESCE((fixed_cost.value ->> 'amount')::numeric, 0)
      END AS amount,
      month_start::date AS event_month,
      to_char(month_start::date, 'YYYY-MM') AS period_key
    FROM anchors
    JOIN public.profiles profile
      ON profile.id = auth.uid()
    CROSS JOIN LATERAL jsonb_array_elements(
      public.cd_normalize_fixed_operational_costs(profile.finance_fixed_operational_costs)
    ) AS fixed_cost(value)
    CROSS JOIN LATERAL generate_series(
      date_trunc('month', (fixed_cost.value ->> 'starts_on')::date)::date,
      LEAST(
        date_trunc(
          'month',
          COALESCE(NULLIF(fixed_cost.value ->> 'ends_on', '')::date, anchors.anchor_month)
        )::date,
        anchors.anchor_month
      ),
      INTERVAL '1 month'
    ) AS month_start
    WHERE COALESCE((fixed_cost.value ->> 'is_active')::boolean, TRUE)
      AND COALESCE((fixed_cost.value ->> 'amount')::numeric, 0) > 0
      AND date_trunc('month', (fixed_cost.value ->> 'starts_on')::date)::date <= anchors.anchor_month
      AND (
        NULLIF(fixed_cost.value ->> 'ends_on', '') IS NULL
        OR date_trunc('month', NULLIF(fixed_cost.value ->> 'ends_on', '')::date)::date
          >= date_trunc('month', (fixed_cost.value ->> 'starts_on')::date)::date
      )
  ),
  all_operational_cost_events AS (
    SELECT operational_cost_events.amount, operational_cost_events.period_key
    FROM operational_cost_events

    UNION ALL

    SELECT fixed_cost_events.amount, fixed_cost_events.period_key
    FROM fixed_cost_events
  ),
  ...
  operational_summary AS (
    SELECT COALESCE(SUM(all_operational_cost_events.amount), 0) AS operational_costs
    FROM all_operational_cost_events
    CROSS JOIN anchors
    WHERE anchors.period_key = 'all'
       OR all_operational_cost_events.period_key = anchors.period_key
  ),
  ...
  chart_operational_summary AS (
    SELECT
      all_operational_cost_events.period_key,
      COALESCE(SUM(all_operational_cost_events.amount), 0) AS operational_costs
    FROM all_operational_cost_events
    GROUP BY all_operational_cost_events.period_key
  ),
  chart_rows AS (
    SELECT
  ...
  period_candidates AS (
    ...
    SELECT all_operational_cost_events.period_key
    FROM all_operational_cost_events
    WHERE all_operational_cost_events.period_key IS NOT NULL
  ),
  top_packages AS (
    SELECT
