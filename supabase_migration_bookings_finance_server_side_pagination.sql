...
$$;
-- Reduce Supabase Disk I/O pressure by splitting listing RPC execution into:
-- 1) lightweight filter/sort/page scan
-- 2) heavy JSON payload build only for paged row ids

CREATE OR REPLACE FUNCTION public.cd_safe_timezone_name(
  p_time_zone TEXT,
  p_fallback TEXT DEFAULT 'UTC'
) RETURNS TEXT
LANGUAGE plpgsql
STABLE
AS $$
DECLARE
  resolved_time_zone TEXT := COALESCE(NULLIF(btrim(p_time_zone), ''), COALESCE(NULLIF(btrim(p_fallback), ''), 'UTC'));
BEGIN
  PERFORM timezone(resolved_time_zone, NOW());
  RETURN resolved_time_zone;
EXCEPTION
  WHEN OTHERS THEN
    RETURN COALESCE(NULLIF(btrim(p_fallback), ''), 'UTC');
END;
$$;
