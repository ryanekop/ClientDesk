...
SET operational_costs = '[]'::jsonb
WHERE operational_costs IS NULL
   OR jsonb_typeof(operational_costs) <> 'array';

CREATE OR REPLACE FUNCTION public.cd_normalize_operational_costs(
...
AS $$
  WITH raw_items AS (
    ...
    FROM jsonb_array_elements(
      CASE
        WHEN jsonb_typeof(p_operational_costs) = 'array' THEN p_operational_costs
        ELSE '[]'::jsonb
      END
    ) AS item(value)
  ),
  normalized AS (
    SELECT
      ...
      GREATEST(
        COALESCE(
          NULLIF(btrim(COALESCE(item ->> 'amount', '')), '')::numeric,
          0
        ),
        0
      ) AS amount,
...
CREATE OR REPLACE FUNCTION public.cd_operational_costs_total(
  p_operational_costs JSONB
) RETURNS NUMERIC
LANGUAGE sql
IMMUTABLE
SET search_path = public
AS $$
  SELECT COALESCE(SUM(COALESCE((item.value ->> 'amount')::numeric, 0)), 0)
  FROM jsonb_array_elements(public.cd_normalize_operational_costs(p_operational_costs)) AS item(value);
$$;

CREATE OR REPLACE FUNCTION public.cd_net_verified_revenue_after_operational_costs(
...
SET search_path = public
AS $$
  SELECT public.cd_net_verified_revenue_amount(
    p_dp_verified_amount,
    p_dp_refund_amount,
    p_final_paid_at,
    p_is_fully_paid,
    p_final_payment_amount
  ) - public.cd_operational_costs_total(p_operational_costs);
$$;

CREATE OR REPLACE FUNCTION public.cd_finance_source_key(
  p_source JSONB
) RETURNS TEXT
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT CASE
    WHEN jsonb_typeof(p_source) <> 'object' THEN 'unknown'
    WHEN COALESCE(p_source ->> 'type', '') = 'cash' THEN 'cash'
    WHEN COALESCE(p_source ->> 'type', '') = 'qris' THEN 'qris'
    WHEN COALESCE(p_source ->> 'type', '') = 'bank'
      THEN CONCAT(
        'bank:',
        COALESCE(NULLIF(btrim(p_source ->> 'bank_id'), ''), 'unknown'),
        ':',
        COALESCE(NULLIF(btrim(p_source ->> 'account_number'), ''), 'na')
      )
    ELSE 'unknown'
  END;
$$;

CREATE OR REPLACE FUNCTION public.cd_finance_source_label(
  p_source JSONB
) RETURNS TEXT
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT CASE
    WHEN jsonb_typeof(p_source) <> 'object' THEN 'Tanpa Sumber'
    WHEN COALESCE(p_source ->> 'type', '') = 'cash' THEN 'Cash'
    WHEN COALESCE(p_source ->> 'type', '') = 'qris' THEN 'QRIS'
    WHEN COALESCE(p_source ->> 'type', '') = 'bank'
      THEN CONCAT(
        COALESCE(NULLIF(btrim(p_source ->> 'bank_name'), ''), 'Transfer Bank'),
        CASE
          WHEN COALESCE(NULLIF(btrim(p_source ->> 'account_number'), ''), '') <> ''
            THEN CONCAT(' • ', btrim(p_source ->> 'account_number'))
          ELSE ''
        END
      )
    ELSE 'Tanpa Sumber'
...
AS $$
  ...
  SELECT jsonb_build_object(
    ...
    'summary', jsonb_build_object(
      ...
      'bookingCount', COALESCE((SELECT booking_count FROM remaining_summary), 0)
    ),
