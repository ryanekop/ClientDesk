CREATE OR REPLACE FUNCTION public.cd_normalize_split_session_date_text(
  p_value TEXT
) RETURNS TEXT
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    ...
    WHEN btrim(p_value) ~ '^\d{4}-\d{2}-\d{2}(?:[T\s].*)?$'
      AND to_char(to_date(LEFT(btrim(p_value), 10), 'YYYY-MM-DD'), 'YYYY-MM-DD') = LEFT(btrim(p_value), 10)
      THEN LEFT(btrim(p_value), 10)
    ELSE NULL
  END;
$$;

CREATE OR REPLACE FUNCTION public.cd_collect_booking_session_filter_dates(
  p_session_date TIMESTAMPTZ,
  p_extra_field_map JSONB,
...
AS $$
  WITH raw_dates AS (
    ...
    SELECT public.cd_normalize_split_session_date_text(
      CASE
        WHEN jsonb_typeof(p_extra_field_map) = 'object' THEN p_extra_field_map ->> 'tanggal_akad'
        ELSE NULL
      END
    )
    UNION ALL
    SELECT public.cd_normalize_split_session_date_text(
      CASE
        WHEN jsonb_typeof(p_extra_field_map) = 'object' THEN p_extra_field_map ->> 'tanggal_resepsi'
        ELSE NULL
      END
    )
    UNION ALL
    SELECT public.cd_normalize_split_session_date_text(
      CASE
        WHEN jsonb_typeof(p_extra_field_map) = 'object' THEN p_extra_field_map ->> 'tanggal_wisuda_1'
        ELSE NULL
      END
    )
    UNION ALL
    SELECT public.cd_normalize_split_session_date_text(
      CASE
        WHEN jsonb_typeof(p_extra_field_map) = 'object' THEN p_extra_field_map ->> 'tanggal_wisuda_2'
        ELSE NULL
      END
    )
    UNION ALL
    SELECT public.cd_normalize_split_session_date_text(
      CASE
        WHEN jsonb_typeof(p_extra_field_map) = 'object' THEN p_extra_field_map ->> 'akad_date'
        ELSE NULL
      END
    )
    UNION ALL
    SELECT public.cd_normalize_split_session_date_text(
      CASE
        WHEN jsonb_typeof(p_extra_field_map) = 'object' THEN p_extra_field_map ->> 'resepsi_date'
        ELSE NULL
      END
    )
    UNION ALL
    SELECT public.cd_normalize_split_session_date_text(
      CASE
        WHEN jsonb_typeof(p_extra_field_map) = 'object' THEN p_extra_field_map ->> 'wisuda_session_1_date'
        ELSE NULL
      END
    )
    UNION ALL
    SELECT public.cd_normalize_split_session_date_text(
      CASE
        WHEN jsonb_typeof(p_extra_field_map) = 'object' THEN p_extra_field_map ->> 'wisuda_session1_date'
        ELSE NULL
      END
    )
    UNION ALL
    SELECT public.cd_normalize_split_session_date_text(
      CASE
        WHEN jsonb_typeof(p_extra_field_map) = 'object' THEN p_extra_field_map ->> 'wisuda_session_2_date'
        ELSE NULL
      END
    )
    UNION ALL
    SELECT public.cd_normalize_split_session_date_text(
      CASE
        WHEN jsonb_typeof(p_extra_field_map) = 'object' THEN p_extra_field_map ->> 'wisuda_session2_date'
        ELSE NULL
      END
    )
  )
  SELECT COALESCE(
    ARRAY(
      ...
      FROM raw_dates AS candidates(candidate_date)
