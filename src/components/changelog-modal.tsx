...
export function DashboardChangelogPopup({
  ...
  return (
    ...
    >
      <DialogContent className="top-[calc(var(--global-announcement-height,0px)+env(safe-area-inset-top,0px)+0.5rem)] flex h-[calc(100dvh-var(--global-announcement-height,0px)-env(safe-area-inset-top,0px)-1rem)] w-[calc(100vw-1rem)] max-w-[min(780px,calc(100vw-1rem))] max-h-[calc(100dvh-var(--global-announcement-height,0px)-env(safe-area-inset-top,0px)-1rem)] flex-col gap-0 translate-y-0 overflow-hidden border-0 bg-transparent p-0 shadow-none sm:top-[50%] sm:h-auto sm:max-h-[85vh] sm:translate-y-[-50%] [&>button]:right-3 [&>button]:top-3 [&>button]:rounded-full [&>button]:bg-background [&>button]:p-2 [&>button]:opacity-100 [&>button]:shadow-sm sm:[&>button]:right-5 sm:[&>button]:top-5">
        <div className="flex min-h-0 flex-1 flex-col rounded-[1.5rem] border border-border/70 bg-background/95 p-3 shadow-[0_30px_80px_-40px_rgba(15,23,42,0.45)] backdrop-blur sm:rounded-[1.75rem] sm:p-5">
          <DialogHeader className="shrink-0 px-1 pb-3 pt-1 text-left sm:px-2 sm:pb-4 sm:pt-2">
          ...
          <div className="mt-3 shrink-0 border-t border-border/70 px-1 pt-3 sm:mt-4 sm:px-2 sm:pt-4">
