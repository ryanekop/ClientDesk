...
type CityAutocompleteItem = CityReferenceItem & {
  display_name?: string;
};

type CityAutocompleteStrings = {
  noResults?: string;
...
type CityAutocompleteProps = {
  ...
  onSelect: (item: CityAutocompleteItem | null) => void;
  placeholder?: string;
  disabled?: boolean;
  ...
  strings?: CityAutocompleteStrings;
};
...
export function CityAutocomplete({
  ...
  const uiStrings = React.useMemo<Required<CityAutocompleteStrings>>(
    () => ({
      noResults: "Kota / kabupaten tidak ditemukan.",
      selectionHint: "Pilih kota/kabupaten dari suggestion agar city_code ikut tersimpan.",
      searchError: "Gagal memuat suggestion kota/kabupaten.",
      ...strings,
    }),
    [strings],
  );
  const resolvedPlaceholder = placeholder || "Cari kota / kabupaten...";
  ...
  const [items, setItems] = React.useState<CityAutocompleteItem[]>([]);
  ...
  React.useEffect(() => {
    function handlePointerDown(event: MouseEvent) {
      ...
      const insidePortal = portalMenuRef.current?.contains(target);
    ...
    const timeout = window.setTimeout(async () => {
      try {
        ...
        const response = await fetch(
          `/api/internal/reference/cities/search?q=${encodeURIComponent(
            searchTerm,
          )}&limit=8`,
          {
            method: "GET",
        ...
        setItems(
          Array.isArray(payload.items)
            ? payload.items.filter(
                (item: unknown): item is CityAutocompleteItem =>
                  Boolean(item) &&
                  typeof item === "object" &&
                  typeof (item as CityAutocompleteItem).city_code === "string" &&
                  typeof (item as CityAutocompleteItem).city_name === "string" &&
                  typeof (item as CityAutocompleteItem).province_name === "string",
              )
            : [],
        );
      } catch (fetchError) {
  ...
  function selectItem(item: CityAutocompleteItem) {
    onValueChange(item.display_name || buildCityDisplayName(item));
    onSelect(item);
    setOpen(false);
    setActiveIndex(-1);
    setError("");
  ...
  const invalidSelection = searchTerm.length > 0 && !selectedCode && !loading;
  const showResults = open && (loading || items.length > 0 || Boolean(error) || invalidSelection);
  ...
  const menuContent = (
    <div className="max-h-[320px] overflow-y-auto rounded-lg border bg-popover shadow-lg">
      {items.map((item, index) => {
        const selected = item.city_code === selectedCode;
        ...
        return (
          ...
          >
            <span className="truncate">
              {item.display_name || buildCityDisplayName(item)}
            </span>
